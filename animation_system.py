...
def create_bay_door_animation(bay_obj, scale=1.0, naming_prefix=''):
    ...
    fc.keyframe_points[0].co = (1.0, 0.0)
    fc.keyframe_points[0].interpolation = 'LINEAR'
    fc.keyframe_points[1].co = (30.0, open_offset)
    fc.keyframe_points[1].interpolation = 'LINEAR'
...
def create_landing_gear_animation(hull_obj, scale=1.0, naming_prefix=''):
    """Create landing-gear proxy objects under *hull_obj* with extend/retract
    animations.

    Three cube proxies are placed beneath the hull: one centre-front and two
    rear-side.  Each receives a Z-location animation that moves it downward
    (extended) and back (retracted).
    ...
    Returns:
        List of ``(gear_obj, action)`` tuples, or an empty list if
        *hull_obj* is *None*.
    """
    if hull_obj is None:
        return []
